<div id="msg_slideshow" class="msg_slideshow">
			...
			<div id="msg_controls" class="msg_controls"><!-- right has to animate to 15px, default -110px -->
				...
				<a href="#" id="msg_link" class="msg_link"></a>		
...
$intimg = ((int)($totalimg/6))*6;

foreach($rss->xpath('channel/item') as $item) {
if($item->img1 != '')
$countimg++;
if ((($countimg % 6) == 1) && ($countimg > 1) && ($item->img1 != ''))
Echo "<div class='msg_thumb_wrapper' style='display:none;'>";
if($item->img1 != '')
Echo "<a href='#'><img src='img/blog/{$item->postid}/{$item->img1}-75.jpg' alt='img/blog/{$item->postid}/{$item->img1}-low.jpg' link='blog.php?no={$item->postid}'/></a>";
if (($countimg  % 6 == 0) && ($countimg > 0) && ($item->img1 != ''))
Echo "</div>";
if($countimg == $intimg)
break;
        ...
        <script type="text/javascript">
            ...
            $(function() {
				...
				*/
				...
				$('#msg_link').bind('click',function(e){
				...
				*/
				...
				function showControls(){
					...
					$('#msg_controls').stop().animate({'right':'20px'},500);

				}

				function hideControls(){

					$('#msg_controls').stop().animate({'right':'-140px'},500);
				...
				*/
				...
				function link(){
					...
					var $thumb = $('#msg_thumbs .msg_thumb_wrapper:nth-child('+current_thumb+')').find('a:nth-child('+ parseInt(current - nmb_images_wrapper*(current_thumb -1)) +')').find('img');

					var source = $thumb.attr('link');
